...
namespace OrderManager.Controllers
{
        ...
        
    {
        ...
        /// </summary>
        /// <param name="clientId">The clientId, in GUID format</param>
        /// <returns>The single client information.</returns>
        [HttpGet("{clientId}")]
        public async Task<ActionResult<ServiceResponse<ClientDetailsDto>>> GetById(Guid clientId)
        {
            var response = await service.GetByIdAsync(clientId);
            if (!response.Success)
            {
        ...
        /// </summary>
        /// <param name="clientId">The client id, in GUID format to be updated.</param>
        /// <param name="clientDto">The client dto object with the information to be edited.</param>
        /// <returns></returns>
        [HttpPut("{clientId}")]
        public async Task<ActionResult<ServiceResponse<ClientDTO>>> Update(Guid clientId, ClientUpdateDto clientDto)
        {
            var response = await service.UpdateAsync(clientId, clientDto);
            if (!response.Success)
            {
                return BadRequest(response);
            }
